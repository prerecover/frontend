...
export default function AdminClinicsBlock({
  clinics,
  countries,
}: {
  clinics: IClinic[];
        ...
        <div className="flex items-center  gap-3 w-full">
          <Select onValueChange={(e) => setCountry(e)} defaultValue={country}>
            <SelectTrigger className="max-w-[340px] py-7 pr-5 pl-6 border-[1px] border-blue-100 bg-[#fff] rounded-[12px]">
              {country === '' && (
                <Text className="text-grey">Местоположение</Text>
              )}
          ...
          <Text className="text-[16px] font-medium min-w-[150px]">
            Работает в выходные ?
          </Text>
      ...
      <Table>
        ...
        <TableBody>
          {clinics
            ...
            .filter((clinic) =>
              clinic.title.toLowerCase().includes(search.toLowerCase())
            )
            .filter((clinic) =>
              country.length > 0
                ? clinic.country?.title == country
            ...
            .map((clinic, i) => (
              <TableRow key={i} className="text-center">
                ...
                <TableCell className="bg-white">
                  <Link
                    className="text-blue"
                    href={`/admin/modification/${clinic._id}`}
                  >
                    Редактировать
                  </Link>
                </TableCell>
