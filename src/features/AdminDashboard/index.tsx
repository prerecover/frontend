...
export default function AdminDasboard({ data: searchData }: { data: ISearch }) {
  const [chunkStats, setChunkStats] = useState(1);
  const { data, loading } = useQuery(STATS_QUERY, {
    variables: { chunk: chunkStats },
  });
  if (loading) {
    return <Loader />;
  }
    ...
    <div className="flex">
      <div className="flex flex-col w-full">
        <div className="flex-between ">
          ...
          <Select
            onValueChange={(e) => setChunkStats(parseInt(e))}
            value={chunkStats.toString()}
          >
