    ...
    return (
        <>
            <Text type='h2' position='center' className='text-[20px] text-center'>
                {prevRoute == '/registration' ? 'Завершение регистрации' : 'Запрос на восстановление'}
            </Text>
            <Text type='p' position='center' className='text-[14px] text-grey-700 '>
                Введите код который был отправлен, для{' '}
                {prevRoute == '/registration' ? 'завершения регистрации' : 'восстановления доступа'}
            </Text>
                                ...
                                <FormControl>
                                    <InputOTP maxLength={4} {...field} pattern={REGEXP_ONLY_DIGITS}>
                                        <InputOTPGroup className='gap-8'>
                                            <InputOTPSlot
                                                index={0}
                                                className={`${error ? 'border-red-400' : data ? 'border-green' : ''}`}
                                            />
                                            <InputOTPSlot
                                                index={1}
                                                className={`${error ? 'border-red-400' : data ? 'border-green' : ''}`}
