...
import {
  IQuestionAnswer,
  ISurveyQuestion,
} from '@/shared/types/survey.interface';
...
export default function SurveyQuestion({
  ...
  const handleDelete = () => {
    ...
    console.log(countEl);
    const copyList = countList.filter(
      (val) => val.getTime() !== countEl?.getTime()
    );
  ...
  return (
    <div className="flex flex-col mt-[22px] cursor-pointer ">
      <div className="flex-between">
        <Text className="text-[16px] font-normal mb-[14px]">
          Вопрос No {pos}
        </Text>
        <Image
      ...
      <div className={` transform duration-300 ease-in-out`}>
        ...
        />
        <Text className="text-[16px] font-normal mt-[18px]">
          Готовые ответы
        </Text>
