...
export default function ChangeAppointmentCard({
  ...
  const availableDatesList = appointment.availableDates.map(
    (avDate) => new Date(avDate.date)
  );
  ...
  const handleMutate = () => {
    ...
    mutate({
      variables: { appointmentId: appointment._id, timeStart: copyDate },
    });
  };
  ...
  const checkCeil = (ceil: string) => {
    ...
    return availableDatesList
      .map((dateEl) => dateEl.getTime())
      .includes(copyDate.getTime());
  };
  useEffect(() => {
    setTime('');
  }, [date]);
