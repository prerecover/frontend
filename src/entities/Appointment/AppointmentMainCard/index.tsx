...
export default function AppointmentMainCard({
  ...
  const doctorSpecialization = `${appointment.doctor?.specialization.title.charAt(0).toUpperCase() + appointment.doctor?.specialization.title.slice(1)}`;
    ...
    <>
      ...
      >
        {appointment.status == 'Rejected' ? (
          <Text
            className="text-red-400 text-[38px] absolute z-40 bottom-28 right-44"
            position="center"
          >
            Отказано
          </Text>
        ...
        >
          ...
          >
            ...
            <div className={cn(pathname.includes('history') && 'hidden')}>
              {appointment.status === 'Approoved' ? (
                <Text className="text-green text-[16px] font-medium">
                  Подтверждена
                </Text>
              ) : appointment.status === 'Pending' ? (
                <Text className="text-[16px] font-medium text-red-400">
                  Выбранное время занято
                </Text>
              ) : appointment.status === 'In process' ? (
                <Text className="text-yellow-400 text-[16px] font-medium">
                  В ожидании
                </Text>
              ) : (
                <></>
          ...
          <Characteristics
            ...
            data={[
              ...
              {
                key: 'Создана:',
                value: `${doctorSpecialization} ${doctorName}`,
              },
              {
                key: 'Версия:',
                value: 'USA',
            ...
            <AlertDialog>
              ...
              <AlertDialogContent className="bg-white border-none rounded-[12px]">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    После отмены запись не подлежит восстановлению, внимательно
                    изучите детали прежде чем отклонить запись
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    ...
                    onClick={() =>
                      reject({ variables: { appointmentId: appointment._id } })
                    }
                  >
                    Отменить
                  </AlertDialogAction>
