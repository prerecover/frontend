...
export default function AppointmentWarn({ appointment }: { appointment: IAppointment }) {
    ...
    return (
        <div
            ...
            <div
                className={cn('bg-yellow-400 transition-all flex justify-between rounded-[10px]')}
                onClick={() => setOpen(!open)}>
                {!open ? (
                    <Text
                        type='p'
                        className={cn('text-[14px] font-medium flex transition-all', open ? 'p-4' : 'px-4 pt-[5px]')}>
                        Выберите другое время
                    </Text>
                ) : (
                    <Text type='p' className={'text-[16px] font-medium flex p-4 transition-all'}>
                        Выберите другое время
                    </Text>
                )}
                <Image
                    src={!open ? '/assets/black-arrow-down.svg' : '/assets/black-arrow-up.svg'}
                    width={20}
                    height={20}
            ...
            {open && (
                <div className='flex flex-col px-4 gap-[20px]'>
                    ...
                    <Text className='font-medium'>Запись на это время занята, выберите другое доступное время</Text>
                    <Button
                        variant={'outline'}
