...
const ClinicsServicesLink = <M extends EnModes>({
  mode,
  data,
  ...props
}: Props<M>) => {
  if (mode === undefined) {
    throw new Error(
      `Некорректный мод для ClinicsServicesLink. Было получение ${mode}, а ожидалось одно из значений EnModes`
    );
  }
