...
export default function ClinicMain({ clinic }: { clinic: IClinic }) {
  const onlineServices = clinic.services?.filter(
    (service) => service.online
  ).length;
  const offlineServices = clinic.services?.filter(
  ...
  return (
    <>
      <div className="desktop:hidden p-4  pc:hidden">
        <div
          className={cn(
            'max-w-none w-full mt-[-16px] mx-[-16px] relative',
            styles.clinic_image
          )}
        >
          <Image
        ...
        <div className="flex items-center gap-2 mt-4 py-[16px] px-0">
          ...
          <Button
            variant={'outline'}
            className="px-10 h-[46px] w-full text-blue border-blue"
          >
            Написать
          </Button>
        </div>
        <LinksBlock
          ...
          doctors={doctors}
        />
        <h1 className="mt-[24px] font-semibold text-[17px] leading-[20px]">
          Общие показатели клиники
        </h1>
        <div className="flex w-full gap-[14px] mb-[14px] ">
          <UnionParams
            title="Опыт в лечении: 3 года"
            treated={clinic.treated}
        ...
        <BoxWrapper
          color="white"
          className="w-full grid grid-cols-2 border-blue-200 gap-8 px-[30px] py-[22px]"
        >
          <div className="flex-between">
            <Text className="font-medium text-[18px] text-grey-700">
              Помощь в лечении
            </Text>
            <Text className="font-medium text-[22px]">0%</Text>
          </div>
          <div className="flex-between">
            <Text className="font-medium text-[18px] text-grey-700">
              Ответственность
