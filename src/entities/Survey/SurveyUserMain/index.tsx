...
export default function SurveyUserMain() {
  ...
  const { surveyUserWindowOpen, survey, setSurveyUserWindowOpen } =
    useSurveyUserWindowStore();
  ...
  useEffect(() => {
    ...
    if (
      survey &&
      survey.questions.length - surveyData.length >= surveyData.length
    ) {
      setProgress('medium');
    }
        ...
        <div className="flex flex-col">
          <div className="flex-center text-[18px] font-medium mt-[17px] gap-1 ">
            ...
            <Text className="text-blue">
              &quot;{survey?.appointment?.title || 'Без названия'}&quot;
            </Text>
            <Image
