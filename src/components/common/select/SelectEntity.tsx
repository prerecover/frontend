...
export default function SelectEntity({ countries }: { countries: ICountry[] }) {
  ...
  const [clinicMutate, { data: clinicData, loading: clinicLoading }] =
    useMutation(SELECT_CLINICS);
  const [serviceMutate, { data: serviceData, loading: serviceLoading }] =
    useMutation(SELECT_SERVICES);
  ...
  return (
    <div className="flex flex-col bg-white p-4">
      <div className="flex flex-col gap-[22px] text-[16px] font-medium">
        <Select
          onValueChange={(value) =>
            value == 'Выбрать услугу' ? setType('service') : setType('clinic')
          }
        >
