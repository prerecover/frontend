...
export const CalendarForApi: FC<ICalendare> = ({
  width,
  height,
  borderColor,
  setDate,
  ...
  const handleSelectedDay = (day: string) => {
    if (
      day === undefined ||
      !dates.map((day) => day.getDate().toString()).includes(day || '')
    )
      return;
    else if (
      day === undefined ||
  ...
  const checkDate = (day: string) => {
    ...
    console.log(
      dates.map((dateEl) => console.log(dateEl.getDate(), dateEl.getMonth()))
    );
    return (
      dates.filter(
        (dateEl) =>
          dateEl.getDate() === parseInt(day) &&
          dateEl.getMonth() === currentMonth
      ).length > 0
    );
  };
  ...
  return (
    <div>
      <div
        className={cn(
          'max-w-full bg-white rounded-[16px] p-7 h-max',
          className
        )}
        style={{
          width,
        ...
        <div className="flex flex-col">
          ...
          <ul className="grid grid-cols-7 justify-between items-center mt-4 gap-2">
            {daysInMonth.map((day, i) => (
              <li
                style={
                  day === undefined ? { cursor: '' } : { cursor: 'pointer' }
                }
                className={cn(
                  today == day &&
