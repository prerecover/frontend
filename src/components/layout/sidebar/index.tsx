...
export default function Sidebar({ className }: { className?: string }) {
  ...
  const hiddenRoutes = ['/create-appointment', '/add-appointment'];
  ...
  return (
    <>
      <section
        className={cn(
          'fixed left-0 top-0 h-dvh w-[258px] bg-white z-10 transition-all duration-300 ease-in border-[1px] border-solid border-blue-100 mobile:hidden tablet:hidden',
          !isOpenSidebar && 'w-[100px]',
          hiddenRoutes.includes(pathname) && 'hidden',
          className
        )}
      >
        ...
        <div
          className={cn(
            'flex flex-col gap-[10px] my-4 mx-0 py-0 px-4',
        ...
        <Text
          type="p"
          className={cn(
            'mt-8 text-grey-700 text-[14px] px-[28px]',
        ...
        <div
          className={cn(
            'flex flex-col h-full px-[28px] mt-[29px] gap-6',
            !isOpenSidebar && 'items-center'
          )}
        >
